...
Most of this is automated via a script in this same directory. The main thing you will need is a GitHub personal access token, which will be used for creating the release in GitHub (so you need write access to the robinovitch61/grpcui repo) and to open a Homebrew pull request.
...
# from the root of the repo
echo v2.3.4 > VERSION
docker build -t robinovitch61/grpcui:v2.3.4 .
# now that we have it built, push to Docker Hub
docker push robinovitch61/grpcui:v2.3.4
# push "latest" tag, too
docker tag robinovitch61/grpcui:v2.3.4 robinovitch61/grpcui:latest
docker push robinovitch61/grpcui:latest
```
...
URL=https://github.com/robinovitch61/grpcui/archive/refs/tags/v2.3.4.tar.gz
